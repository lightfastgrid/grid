...
export function applyAdvancedExportPlacement(
  ...
  const panel =
    root.closest(".interactive-demo-panel") || root;
  const rect = panel.getBoundingClientRect();
  const position = resolveAdvancedExportFixedPosition({
    panelTop: rect.top,
  ...
  flyout.style.width = Math.round(position.width) + "px";
  flyout.style.maxWidth = Math.round(position.width) + "px";
